...
(function () {
    ...
    xhr.open('GET', 'bootstrap.json?_dc=' + (application.debug ? (+new Date()) : application.JSVersion), false);
    xhr.send(null);
    ...
    for (i = 0, ln = styleSheets.length; i < ln; i++) {
        ...
        if (typeof path != 'string') {
            ...
            theme = path.theme;
            path = path.path + '?_dc=' + (application.debug ? (+new Date()) : application.JSVersion);
        }
