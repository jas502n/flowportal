...
Ext.define('YZSoft$Boot.src.plugin.ListOptions', {
    ...
    showOptions: function (target, racord) {
        ...
        Ext.DomHelper.append(target.element.dom, '<div class="yzlg-list-option-wrap"></div>');

        target.optionWrap = target.element.down('.yzlg-list-option-wrap');
        target.optionContainer = me.createOptions({
            height: target.optionWrap.getHeight(true)
        });
        target.optionContainer.renderTo(target.element.down('.yzlg-list-option-wrap'));
    },
    ...
    createOptions: function (config) {
        ...
        return Ext.create('Ext.Container', Ext.apply({
            ...
            defaults: {
                xtype: 'button',
                cls: 'yzlg-button-flat yzlg-button-list-option'
            },
            items: items
        }, config));
    },
