    ...
    public class ReportHandler : YZServiceHandler
    {
        ...
        public virtual JObject GetReportsInFolder(HttpContext context)
        {
            ...
            using (BPMConnection cn = new BPMConnection())
            {
                ...
                foreach (Report tmpReport in reports)
                {
                    string fullName;
                    if (String.IsNullOrEmpty(path))
                        fullName = tmpReport.Name;
                    else
                        fullName = path + "/" + tmpReport.Name;

                    JObject item = new JObject();
                    children.Add(item);

                    item["Name"] = tmpReport.Name;
                    item["DataSourceName"] = tmpReport.DataSourceName;
                    item["FullName"] = fullName;
                    item["rsid"] = StoreZoneType.Reports.ToString() + "://" + fullName;

                    JArray jViews = new JArray();
                    item["Views"] = jViews;

                    foreach (ReportView view in tmpReport.Views){
                        JObject jView = new JObject();
                        jViews.Add(jView);
                        jView["ViewType"] = view.ViewType.ToString();
                    }
                }
        ...
        public virtual JObject GetReportDefine(HttpContext context)
        {
            ...
            if (!String.IsNullOrEmpty(report.ExportTemplateFile))
                rv["ExportTemplate"] = Path.Combine("YZSoft/Report/Templates", report.ExportTemplateFile);
        ...
        public virtual object GetExcelViewDefine(HttpContext context)
        {
            ...
            return new
            {
                ...
                view = reportView,
                gridViewName = this.GetFirstGridViewName(report),
                template = Path.Combine("YZSoft/Report/Templates", reportView.TemplateFile)
            };
        }
