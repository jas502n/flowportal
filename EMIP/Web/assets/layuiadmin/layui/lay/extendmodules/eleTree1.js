...
layui.define(["jquery", "laytpl", "layer", "form"], function (exports) {
    ...
    function Class(option) {
        ...
        this.isSpread = this.option.isSpread;
    ...
    Class.prototype = {
        ...
        render: function () {
            ...
            self.isSpread = self.option.isSpread == null ? true : self.option.isSpread;
                        ...
                        , '<span class="eleTree-node-content-icon">'
                            ...
                            , (function () {
                                ...
                                if (val.children && val.children.length > 0) {
                                    ...
                                    var icon = '<i class="layui-icon layui-icon-triangle-r" style="color: transparent;"></i>';
                                    if (!self.showCheckbox) {
                                        icon += '<i class="layui-icon10">&#xe621;</i>';
                                    }
                                    return icon;
                                }
                            })()
        ...
        eleTreeEvent: function () {
            ...
            $(this.elem).on("click", ".eleTree-node-content", function (e) {
                ...
                // 数据返回
                layui.event.call(this, "eleTree", 'itemclick(' + self.filter + ')', {
                    data: self.data
                    , currentData: d
                });
            ...
            $(this.elem).on("dblclick", ".eleTree-node-content", function (e) {
                if (self.isSpread) {
                    e.stopPropagation();
                    var el = $(this).children(".eleTree-node-content-icon").children(".layui-icon");
                    if (el.hasClass("icon-rotate")) {
            ...
            $(this.elem).on("click", ".layui-icon-triangle-r", function (e) {
                e.stopPropagation();
                var el = $(this);
                if (el.hasClass("icon-rotate")) {
                    // 合并
                    $(this).parent().parent().siblings(".eleTree-node-group").children().hide("fast");
                    el.removeClass("icon-rotate");
                    el.next().html("&#xe622;");
                } else {
                    $(this).parent().parent().siblings(".eleTree-node-group").children().show("fast");
                    el.addClass("icon-rotate");
                    el.next().html("&#xe624;");
                }
            });
        ...
        getCheckedData: function () {
            ...
            $(this.elem + " input[eletree-status='2']").each(function (index, item) {
                var node = $(item).parent(".eleTree-node-content").parent(".eleTree-node ");
                var d = self.reInitData(node).currentData;
                self.checkedData.push(d);
            })
