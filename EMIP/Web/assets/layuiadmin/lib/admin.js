...
layui.define('view', function (exports) {
    ...
    var events = admin.events = {
      ...
      , refresh: function () {
          ...
          var divtab = admin.tabsBody(admin.tabsPage.index);

          var iframe = divtab.find(ELEM_IFRAME);
          if (iframe.length > 0) {
              iframe[0].contentWindow.location.reload(true);
          } else if (divtab.children().hasClass('yz-func-panel') && divtab.children().children().length > 0) {
              var Extid = divtab.children().children().children().children();
              if (Extid.length > 0 && Extid.attr('id') != null) {
                  var g = Ext.getCmp(Extid.attr('id'));
                  if (g && g.store)
                      g.store.reload();
              }
          }
      ...
      , about: function () {
          admin.popupRight({
            ...
            , success: function () {
                admin.req({
                    url: '/admin/Portal/Data/GetServerInfo.ashx'
                    , type: 'get'
                    , success: function (res) {
                        view('LAY_adminPopupAbout').render('system/about', res);
                    }
                });
    ...
    element.on('tabDelete(' + FILTER_TAB_TBAS + ')', function (obj) {
        ...
        var tabthis = admin.tabsBody(obj.index);
        if (tabthis.children().children().length > 0) {
            var cmp = Ext.getCmp(tabthis.children().children().attr('id'));
            if (cmp)
                cmp.destroy();
        }
        ...
        var othis = admin.tabsBody(othis.index());
        if (othis.children().hasClass('yz-func-panel') && othis.children().children().length > 0) {
            var cmp = Ext.getCmp(othis.children().children().attr('id'));
            if (cmp) {
                var h = $('.layui-show .yz-func-panel').height() + 15;
                cmp.height = h;
                cmp.doLayout();
            }
        }
    ...
    $body.on('click', '*[lay-href]', function () {
        ...
        , nid = othis.attr('lay-nid')
        , pageType = othis.attr('lay-pageType')
        , router = layui.router();

        admin.tabsPage.elem = othis;
        //admin.prevRouter[router.path[0]] = router.href; //记录上一次各菜单的路由信息

        if (othis.find(".layui-badge").length > 0) { //点击菜单删除徽章
            othis.find(".layui-badge").remove();
        }
        //执行跳转
        var topLayui = parent === self ? layui : top.layui;
        topLayui.index.openTabsPage(nid, pageType, href, text || othis.find("cite").text());
    });
    ...
    $body.on('mouseenter', '*[lay-tips]', function () {
        var othis = $(this);
        if (othis.parent().hasClass('layui-nav-item') && !othis.parents('.layui-header').length > 0 && !container.hasClass(SIDE_SHRINK)) return;
